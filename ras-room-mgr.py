...
class RASRoomManager:
    ...
    def _load_config(self, config_file):
        ...
        if not os.path.exists(config_file):
            print(f'Config file {config_file} not found, using default URL: '
                  f'{default_url}')
            return default_url
        ...
        try:
            ...
            if 'server' in config and 'base_url' in config['server']:
                ...
                print(f'Warning: Invalid config format in {config_file}, '
                      f'using default URL')
                return default_url
    ...
    def get_chat_rooms(self, room_type):
        ...
        try:
            ...
            print(f'  Make sure the retro AIM server is running at: '
                  f'{self.base_url}')
            return None
            ...
            if room.create_time:
                try:
                    dt = datetime.fromisoformat(
                        room.create_time.replace('Z', '+00:00')
                    )
    ...
    def create_chat_room(self, chat_room):
        ...
        if chat_room.type == 'private':
            print(f'Error: Creating private chat rooms is not supported by '
                  f'the server.')
        ...
        try:
            ...
            response = requests.post(rooms_endpoint, json=payload,
                                   headers=headers)
            
            if response.status_code == 201:
                print(f'{chat_room.type.capitalize()} chat room '
                      f'\'{chat_room.name}\' created successfully!')
                return True
                ...
                print(f'{chat_room.type.capitalize()} chat room '
                      f'\'{chat_room.name}\' already exists.')
                return False
            else:
    ...
    def delete_chat_room(self, chat_room):
        ...
        try:
            ...
            check_query = 'SELECT COUNT(*) FROM chatRoom WHERE name = \'' + \
                         escaped_name + '\''
            ...
            if room_count == 0:
                print(f'{chat_room.type.capitalize()} chat room '
                      f'\'{chat_room.name}\' deleted successfully!')
                return True
            else:
                print(f'Warning: Room \'{chat_room.name}\' may not have '
                      f'existed or deletion failed.')
                return False
    ...
    def _check_database_permissions(self):
        ...
        try:
            ...
            if not is_readable or not is_writable:
                ...
                read_access = 'Yes' if is_readable else 'No'
                write_access = 'Yes' if is_writable else 'No'
                print(f'  Read access: {read_access}')
                print(f'  Write access: {write_access}')
                print(f'')
                print(f'  Possible solutions:')
                print(f'  - Run the script as a user with database access '
                      f'(e.g., sudo)')
                print(f'  - Change file permissions: sudo chmod 666 '
                      f'{self.sqlite_path}')
...
Examples:
  ...
  %(prog)s --config-file myserver.ini              # Use custom config file
    ...
    parser.add_argument(
        ...
        help='Action to perform: \'get\' to list rooms, \'create\' to create '
             'a new room, \'delete\' to delete a room'
    )
    ...
    parser.add_argument(
        'room_name',
        nargs='?',
        help='Name of the room to create or delete (required for \'create\' '
             'and \'delete\' actions)'
    )
    ...
    if args.action == 'get':
        ...
        if rooms is not None:
            print(f'\nSuccessfully retrieved {len(rooms)} {args.room_type} '
                  f'chat room(s).')
            sys.exit(0)
        else:
        ...
        if not room_name:
            ...
            print(f'\nUsage examples:')
            print(f'  python ras-room-mgr.py create public '
                  f'"My Public Room"')
            sys.exit(1)
        ...
        if success:
            print(f'\nSuccessfully created {args.room_type} chat room: '
                  f'\'{room_name}\'')
            print(f'\nYou can now:')
            print(f'- Run \'python ras-room-mgr.py get {args.room_type}\' '
                  f'to verify the room was created')
            print(f'- Connect with an AIM client to join the room')
            sys.exit(0)
        else:
            print(f'\nFailed to create {args.room_type} chat room: '
                  f'\'{room_name}\'')
            sys.exit(1)
        ...
        if not room_name:
            ...
            print(f'\nUsage examples:')
            print(f'  python ras-room-mgr.py delete public '
                  f'"Room to Delete"')
            print(f'  python ras-room-mgr.py delete private '
                  f'"Private Room to Delete"')
            sys.exit(1)
        ...
        if success:
            print(f'\nSuccessfully deleted {args.room_type} chat room: '
                  f'\'{room_name}\'')
            print(f'\nYou can now:')
            print(f'- Run \'python ras-room-mgr.py get {args.room_type}\' '
                  f'to verify the room was deleted')
            sys.exit(0)
        else:
            print(f'\nFailed to delete {args.room_type} chat room: '
                  f'\'{room_name}\'')
            sys.exit(1)
