...
class RASRegressionTest:
    ...
    def test_list_rooms(self):
        ...
        if (result.returncode == 0 and 
            'Successfully retrieved' in result.stdout):
    ...
    def run_full_regression_test(self):
        ...
        def test_room_not_exists():
            return self.check_room_exists(self.test_room_name,
                                        should_exist=False)
        self.run_test('Verify Test Room Doesn\'t Exist Initially',
                     test_room_not_exists)
        
        # Test 3: Create test room
        create_success = self.run_test('Create Public Room',
        ...
        if create_success:
            ...
            def test_room_exists():
                return self.check_room_exists(self.test_room_name,
                                            should_exist=True)
            self.run_test('Verify Room Exists After Creation',
                         test_room_exists)
            
            # Test 5: List rooms again (should show our new room)
            self.run_test('List Public Rooms (With Test Room)',
                         self.test_list_rooms)
            
            # Test 6: Delete the test room
            delete_success = self.run_test('Delete Public Room',
            ...
            if delete_success:
                ...
                def test_room_deleted():
                    return self.check_room_exists(self.test_room_name,
                                                should_exist=False)
                self.run_test('Verify Room Deleted Successfully',
                             test_room_deleted)
                
                # Test 8: Final room list (should not show our room)
                self.run_test('List Public Rooms (Final)',
                             self.test_list_rooms)
